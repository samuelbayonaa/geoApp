...
export async function createRestaurant(form) {
  try {
    ...
    const newRestaurant = await databases.createDocument(
      ...
      restaurantId, // Usamos el restaurantId como ID único del documento
      {
        restaurantId: restaurantId, // Guardamos el restaurantId en un campo adicional
        name: form.name,
        direction: form.direction,
        menu: form.menu,
        type: form.type,
        image: imageUrl,
        creator: form.userId, // ID del usuario que crea el restaurante
      }
    );

    return newRestaurant; // Retorna el nuevo restaurante creado
  } catch (error) {
    throw new Error(error.message); // Propagamos el error si ocurre
  }
}
...
export async function createReview(form, restaurantId, userId) {
  try {
    const newReview = await databases.createDocument(
      config.databaseId,
      config.reviewsCollectionId, // ID de la colección de reseñas
      ID.unique(), // Generar un ID único
      {
        restaurantId: restaurantId, // Vincula la reseña con el ID del restaurante
        userId: userId, // Vincula la reseña con el ID del usuario
